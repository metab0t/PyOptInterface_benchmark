...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i <= m; i++)
  {
    ...
    for (int j = 0; j <= n; j++)
    {
      y[i].push_back(model.addVar(0.0, 1.0, 0.0, GRB_CONTINUOUS));
    }
  }
  vector<GRBVar> u;
  u.push_back(model.addVar(0.0, 1.0, 0.0, GRB_CONTINUOUS));
  for (int i = 1; i <= m; i++)
  {
    u.push_back(model.addVar(-1.0, 1.0, 0.0, GRB_CONTINUOUS));
  }
  model.update();
  // objective
  GRBQuadExpr term1;
